<template>
    <div>
        <template v-if="'properties' in objectForCard && 'type' in objectForCard.properties">
            ...
            <v-row justify="start">
                ...
                <v-col cols="2" sm="6" md="5" lg="6">
                    <v-btn class="ma-0 pa-0 btn_on_card" :disabled="infoCardOn.data" width="100%" depressed
                        color="#EE5E5E" @click="addMainAttribute">
        ...
        <template v-if="'properties' in objectForCard && 'type' in objectForCard.properties">
            ...
            <v-row justify="start">
                <v-col v-for="(el, index) in objectForCard.properties.properties" :key="index" cols="2" sm="6" md="5"
                    lg="6" v-show="el.text != 'id'">
                ...
                <v-col cols="2" sm="6" md="5" lg="6">
                    <v-btn class="ma-0 pa-0 btn_on_card" :disabled="infoCardOn.data" width="100%" depressed
                        color="#EE5E5E" @click="addAdditionalAttribute">
                        <v-icon color="white" dark>
                            mdi-plus
                        </v-icon>
                    </v-btn>
        ...
        <template v-if="'name' in objectForCard">
            ...
            <v-row justify="start">
                <v-col v-for="(el, index) in typeForFeature.properties" :key="index" cols="2" sm="6" md="5" lg="6"
                    v-show="el.text != 'id'">
                    <v-text-field v-model="objectForCard_.properties[el]" hide-details :label="el" :placeholder="el"
                        filled :readonly="infoCardOn.data">
                    </v-text-field>
                </v-col>
            </v-row>
        </template>
